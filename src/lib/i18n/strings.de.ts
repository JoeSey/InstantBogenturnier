...
export const strings = {
  ...
  scoring: {
    ...
    // v2 — 3D single-archer card + outcome picker.
    columnOutcome: 'Wertung',
    columnPoints: 'Punkte',
    outcomeMiss: 'Fehlschuss',
    outcomeNotScored: '—',
    outcomeArrowHeader: (n: number) => `${n}. Pfeil`,
    outcomeZoneOrdinal: (zone: string, ordinal: number) => `${zone} (${ordinal}.)`,
    outcomePickerTitle: (name: string, station: string) => `${name} — Ziel ${station}`,
    outcomeAria: (label: string, points: number) => `${label}, ${points} Punkte`,
    columnLine: 'Linie',
    columnName: 'Name',
    columnClass: 'Klasse',
    columnSum: 'Summe',
