...
// 3D outcome zones (Kill/Vital/Wound) use their own three-colour scheme in
// ScoreOutcomeGrid; the card cells mirror it, lightened the same way.
export function outcomeCellColorClass(zone: string | null): string {
  if (zone === 'K') return 'bg-teal-100 text-slate-900 dark:bg-teal-400/25 dark:text-teal-50';
  if (zone === 'V') return 'bg-amber-100 text-slate-900 dark:bg-amber-400/25 dark:text-amber-50';
  if (zone === 'W') return 'bg-orange-100 text-slate-900 dark:bg-orange-400/25 dark:text-orange-50';
  return 'bg-gray-200 text-slate-700 dark:bg-gray-500/30 dark:text-gray-100';
}
